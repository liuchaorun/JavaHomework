package cn.liuchaorun.secondHomework.seventh;

import cn.liuchaorun.secondHomework.seventh.Q2Resources.*;
...
public class TextZip {
	...
	//ID, 该学号的值需要修改!
	private static final String ID = "201692248";
  ...
  /**
	...
	*/
	public static ArrayList countFrequencies(FileReader fr, PrintWriter pw) throws Exception {
        ...
        while (fr.read(c)==1){
            temp.merge(c[0], 1, (a, b) -> a + b);
        }
        ArrayList<TreeNode> arrayList = new ArrayList<>();
        Set s = temp.keySet();
        Iterator it = s.iterator();
        while (it.hasNext()){
            char t = (Character) it.next();
            arrayList.add(new TreeNode(new CharFreq(t, temp.get(t))));
            pw.println(t+" "+temp.get(t));
        }
        return arrayList;
	}
  ...
  /**
	...
	*/
	public static TreeNode buildTree(ArrayList trees) throws IOException {
		// IMPLEMENT THIS METHOD
        while(trees.size() != 1){
            TreeNode min1 = removeMin(trees);
            TreeNode min2 = removeMin(trees);
            int min1F = ((CharFreq)min1.getItem()).getFreq();
            int min2F = ((CharFreq)min2.getItem()).getFreq();
            if(min1F < min2F){
                trees.add(new TreeNode(new CharFreq('\u0000', min1F+ min2F),min1,min2));
            }
            else if(min1F == min2F){
                if(((CharFreq)min2.getItem()).getChar() == '\u0000'){
                    trees.add(new TreeNode(new CharFreq('\u0000', min1F+ min2F),min1,min2));
                }
                else{
                    if(((CharFreq)min1.getItem()).getChar() == '\u0000'){
                        trees.add(new TreeNode(new CharFreq('\u0000', min1F+ min2F),min2,min1));
                    }
                    else{
                        trees.add(new TreeNode(new CharFreq('\u0000', min1F+ min2F),min1,min2));
                    }
	...
	public static void generatorTable(TreeMap<Character,String> table,TreeNode t,String code){
		if (t.getLeft() != null){
			generatorTable(table, t.getLeft(),code+"0");
		}
		if(t.getRight() != null){
			generatorTable(table, t.getRight(),code+"1");
		}
		if(t.getRight() == null || t.getLeft() == null){
			table.put(((CharFreq)(t.getItem())).getChar(),code);
		}
    }
  ...
  /**
	...
	*/
	public static void compress(FileReader fr, TreeNode huffman, BitWriter bw) throws Exception {
		// IMPLEMENT THIS METHOD
        TreeMap<Character,String> table = new TreeMap<>();
        generatorTable(table,huffman,"");
        char[] c = new char[1];
        while (fr.read(c) == 1){
            String code = table.get(c[0]);
            for (int i = 0; i < code.length(); i++){
                bw.writeBit(code.charAt(i) == '1');
            }
        }
	}
...
//            char c = s.charAt(0);
//            int freq = Integer.parseInt(s.substring(2,s.length()));
//            arrayList.add(new TreeNode(new CharFreq(c,freq)));
//        }
		char[] c = new char[1];
		while (fr.read(c) == 1){
			...
			arrayList.add(new TreeNode(new CharFreq(c[0],Integer.parseInt(stringBuilder.toString()))));
		}
        return arrayList;
	}
	...
	*/
	...
	public static void main(String[] args) throws Exception {
		...
		if (args[0].equals("-a")) {
            ...
            System.out.println("Compressed ratio: "+ f1.length()*1.0/f2.length()+"%");
			...
			System.out.println("file.txt compressed by " + ID);
			FileReader fr = new FileReader(args[1]);
			PrintWriter pw = new PrintWriter(new FileWriter(args[2]));
			ArrayList trees = countFrequencies(fr, pw);
			...
			FileReader frr = new FileReader(args[1]);
			...
			System.out.println("file.txz decompressed by " + ID);
